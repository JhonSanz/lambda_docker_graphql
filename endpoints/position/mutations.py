...
class Mutation:
	@strawberry.field
	def add_position(self, open_date: str, close_date: str, price: float, volume: float, is_leveraged: bool, order_type: str, direction: str, description: str) -> Position:
		id = str(uuid.uuid1())
		data = {
			'id': id,
			'open_date': open_date,
	...
	@strawberry.field
	def update_position(self, id: str, open_date: str, close_date: str, price: float, volume: float, is_leveraged: bool, order_type: str, direction: str, description: str) -> Position:
		to_update = {}
		if open_date: to_update[':open_date'] = open_date
		if close_date: to_update[':close_date'] = close_date
		if price: to_update[':price'] = price
		if volume: to_update[':volume'] = volume
		if is_leveraged: to_update[':is_leveraged'] = is_leveraged
		if order_type: to_update[':order_type'] = order_type
		if direction: to_update[':direction'] = direction
